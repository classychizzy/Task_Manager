...
export class Auth_Controller {
    ...
    public async loginUser(req: Request, res: Response) {
        try {
            ...
            // this ensures that tokens are not released to the frontend
            if (result.data?.accessToken) {
                // store tokens in secure httponly cookies so they cant
                // be accessed by client side js
                res.cookie('accessToken', result.data.accessToken, {
                    httpOnly: true,
                    secure: process.env.NODE_ENV === 'production', // requires HTTPS in prod
                    sameSite: 'lax',
                    maxAge: 15 * 60 * 1000, // 15 min, match your access token expiry
                });
                res.cookie('refreshToken', result.data.refreshToken, {
                    httpOnly: true,
                    secure: process.env.NODE_ENV === 'production',
                    sameSite: 'lax',
                    maxAge: 24 * 60 * 60 * 1000, // 24h, match your refresh token expiry
                    path: '/api/v1/auth/refresh', // only sent to the refresh endpoint
                });
                // strip tokens out of the JSON body — only user data goes to the frontend
                const { accessToken, refreshToken, ...rest } = result.data;
                return res.status(result.status_code || 200).json({
                    ...result,
                    data: rest,
                });
            }
