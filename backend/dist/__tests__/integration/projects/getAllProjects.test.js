...
describe('Get All Projects Integration Tests', () => {
    ...
    describe('GET /api/v1/projects/all', () => {
        ...
        it('should return only the authenticated user\'s own projects', async () => {
            ...
            expect(response.body.data.every((p) => p.user.user_id === userA.userId)).toBe(true);
        });
        ...
        describe('pagination', () => {
            ...
            it('should return the second page correctly with no overlap', async () => {
                ...
                const pageOne = await (0, supertest_1.default)(server)
                    .get('/api/v1/projects/all?page=1&limit=2')
                    .set('Authorization', `Bearer ${accessToken}`);
                const pageTwo = await (0, supertest_1.default)(server)
                    .get('/api/v1/projects/all?page=2&limit=2')
                    .set('Authorization', `Bearer ${accessToken}`);
                const pageOneIds = pageOne.body.data.map((p) => p.project_id);
                const pageTwoIds = pageTwo.body.data.map((p) => p.project_id);
